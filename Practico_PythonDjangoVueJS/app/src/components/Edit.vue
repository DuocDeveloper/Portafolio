...
export default {
    ...
    mounted() {
        axios.get('http://localhost:8000/api/subcripciones' + this.$route.params.id)
        .then(response => {
            console.log(response.data)
            this.subcripcion = response.data
        })
    },
    methods: {
        update: function(e) {
            this.$validator.validate().then(result => {
                ...
                axios.put('http://localhost:8000/api/subcripciones/${this.subcripcion.id}', this.subcripcion)
                .then(response => {
                    this.$router.push('/');
                })
            });
